...
#Check if there are wires with same transfert speed
    execute as @e[type=minecraft:glow_item_frame,tag=!EF_Connected,tag=EF_Wire,distance=..1.1] if score @s EF_Watt >= MaxSpeed EF_MaxSpeed at @s run function energy_flux:send/found/samewire

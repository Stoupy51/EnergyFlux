...
#Elsif there are wires without the same transfert speed
    #Check the highest transfert speed of wires plugged to start
        execute if entity @e[type=#energy_flux:entities,tag=!EF_Connected,tag=EF_Wire,distance=..1.1] run function energy_flux:send/checkspeeds/1
    #Start checking connectivity
        execute as @e[type=#energy_flux:entities,tag=!EF_Connected,tag=EF_Wire,tag=EF_Faster,distance=..1.1] at @s run function energy_flux:send/found/notsamewire
